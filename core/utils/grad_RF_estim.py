...
def GAN_grad_RF_estimate(G, model, target_layer, target_unit, input_size=(3,227,227),
                     device="cuda", show=True, reps=200, batch=1, gan_type="fc6"):
    # (slice(None), 7, 7)
    handle, module_names, module_types = register_hook_by_module_names(target_layer,
        get_activation("record", target_unit, ingraph=True), model,
        input_size, device=device, )
    ...
    for i in range(reps):
        if gan_type == "fc6":
            ...
            intsr = G.visualize(torch.randn(batch, 256).cuda()) * 2 - 1
        else:
            raise NotImplementedError("GAN type %s not implemented"%gan_type)
...
def fit_2dgauss(gradAmpmap_, pop_str=None, outdir=None, plot=False, show=False):
