...
class TorchScorer:
    ...
    def __init__(self, model_name, imgpix=227, rawlayername=True):
        ...
        if isinstance(model_name, torch.nn.Module):
            ...
            if model_name == "vgg16":
                ...
                self.inputsize = (3, imgpix, imgpix)
            elif model_name == "alexnet_untrained":
    ...
    def cleanup(self, print_info=True, record_layers_cleaning=True, recordings_cleaning=True, activation_cleaning=True):
        if print_info: print("Cleanuping...")
        for hook in self.hooks:
            hook.remove()
            self.hooks.remove(hook)
        self.hooks = []
        if print_info: print("Cleanup hooks done.")
        if record_layers_cleaning:
            self.record_layers = []
        if recordings_cleaning:
            self.recordings = {}
        if activation_cleaning:
            self.activation = {}
        if print_info: print("Cleanup done.")        
...
def resize_and_pad_tsr(img_tsr, size, offset, canvas_size=(227, 227), scale=1.0):
    ...
    imgn = img_tsr.shape[0]
    # if image_tsr size was not the same as canvas_size, resize it.
    if img_tsr.shape[2:] != canvas_size:
        img_tsr = F.interpolate(img_tsr, size=canvas_size)
