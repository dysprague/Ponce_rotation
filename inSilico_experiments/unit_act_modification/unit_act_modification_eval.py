...
parser.add_argument("--score_method", type=str, default=["cosine", "MSE", "Correlation"], nargs="+", 
...
if sys.platform == "linux":
    exp_result_main_root = r"/n/scratch3/users/a/ala2226/unit_inflation_small_set_081823"
    refimgdir = r"/n/scratch3/users/a/ala2226/insilico_experiments/tiny_set_image"
    sys.path.append(r"/home/ala2226/Cosine-Project") 
else:
    exp_result_main_root = r"C:\Data\cosine\insilico_experiments\unit_suppressing_eval_t12"
...
if __name__=="__main__":
    ...
    expriment_meta_data_df = pd.DataFrame(columns=["output_type", "trget_imge_name", "similarity_metric", "pop_size", "pop_resampling_id",
                                       ...
                                       "pop_unit_idx", "sub_pop_type", "gen_rerun_id", "modif_unit_idx", "modif_unit_id", "exp_stage"])
        ...
        for pop_resampling_id in range(NUM_OF_RUNS_PER_POP_SIZE):
            ...
            for score_method in metric_name:
                ...
                for gen_rerun_id in range(NUM_OF_RUNS_PER_IMAGE):
                    ...
                    trial_param_dict["exp_stage"] = "reconstruction"
                    ## set fig title 
                    ...
                    # get act mat for the best generated image
                    actmat_best_gen = np.expand_dims(actmat_all[scores_all.argmax()], axis=0)
                    imgcode_best_gen = np.expand_dims(codes_all[scores_all.argmax()], axis=0)
                    pop_act_abs_mean = np.mean(np.absolute(actmat_best_gen))
                    # kill the units
                    for unit_idx in range(popsize*len(layer_name)):
                        trial_param_dict["modif_unit_idx"] = unit_idx
                        trial_param_dict["modif_unit_id"] = (layer_name[unit_idx//popsize], unit_mask_dict[layer_name[unit_idx//popsize]][unit_idx%popsize])
                        trial_param_dict["exp_stage"] = "inflation"
                        
                        #set objective function
                        objfunc = set_objective('inflate_unit', actmat_best_gen, unit_id=unit_idx, popul_mask=None, normalize=False)
                        ## set optimizer
                        optimizer = CholeskyCMAES(code_length, population_size=None, init_sigma=3,
                                        init_code=imgcode_best_gen, Aupdate_freq=10,
                                        maximize=True, random_seed=None, optim_params={})
                        
                        codes_all_stage2, scores_all_stage2, actmat_all_stage2, generations_stage2,\
                        last_gem_img_stage2, last_gem_img_maske_stage2, best_imgs_stage2, best_imgs_RF_masked_stage2, best_scores_stage2, best_codes_stage2 =\
                            run_evol(scorer, objfunc, optimizer, G, reckey=layer_name, savedir=exp_result_root,
                                    meta_data_df=expriment_meta_data_df, trial_param_dict=trial_param_dict, init_code=imgcode_best_gen,
                                    titlestr =title_str, steps=args.steps, RFresize=True, corner=pading_size, imgsize=img_size,
                                    RF_mask=fitdict["fitmap"], save_plot="full", save_data=True)
                        
                        figh = visualize_single_cell_modification(actmat_all_stage2 , generations_stage2 , unit_idx, titlestr=title_str)
                        print_fig(figh, exp_result_root, expriment_meta_data_df, output_type= "unit_opt",**trial_param_dict) 
                        actmat_best_gen_after_sup = np.expand_dims(actmat_all_stage2[scores_all_stage2.argmax()], axis=0)
                        print(f'value of suppresed unit after optimization = {actmat_best_gen_after_sup[0,unit_idx]}')
                        
                        #-----

                        trial_param_dict["exp_stage"] = "inflation_from_scratch"
                        #set objective function
                        objfunc = set_objective('inflate_unit', actmat_best_gen, unit_id=unit_idx, popul_mask=None, normalize=False)
                        ## set optimizer
                        optimizer = CholeskyCMAES(code_length, population_size=None, init_sigma=3,
                        ...
                        codes_all_stage3, scores_all_stage3, actmat_all_stage3, generations_stage3,\
                        last_gem_img_stage3, last_gem_img_maske_stage3, best_imgs_stage3, best_imgs_RF_masked_stage3, best_scores_stage3, best_codes_stage3 =\
                            run_evol(scorer, objfunc, optimizer, G, reckey=layer_name, savedir=exp_result_root,
                                    meta_data_df=expriment_meta_data_df, trial_param_dict=trial_param_dict,
                                    titlestr =title_str, steps=args.steps, RFresize=True, corner=pading_size, imgsize=img_size,
                                    RF_mask=fitdict["fitmap"], save_plot="full", save_data=True)
                        
                        figh = visualize_single_cell_modification(actmat_all_stage3 , generations_stage3 , unit_idx, titlestr=title_str)
                        print_fig(figh, exp_result_root, expriment_meta_data_df, output_type= "unit_opt",**trial_param_dict) 
                        actmat_best_gen_after_sup = np.expand_dims(actmat_all_stage3[scores_all_stage3.argmax()], axis=0)
                        print(f'value of suppresed unit after optimization from scrtch = {actmat_best_gen_after_sup[0,unit_idx]}')
                    
                    trial_param_dict["modif_unit_idx"] = None
                    trial_param_dict["modif_unit_id"] = None
